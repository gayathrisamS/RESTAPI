...
@RestController
public class GayathriCourseController {
    @Autowired
    private CourseService ser;

    @PostMapping("/api/course")
    public ResponseEntity<Course> post(@RequestBody Course person) {
        if (ser.post(person)) {
    ...
    public ResponseEntity<List<Course>> getbyAge(@PathVariable String courseName) {
        List<Course> li = ser.getbyAge(courseName);
        if (li.size() > 0) {
            return new ResponseEntity<>(li, HttpStatus.OK);
        } else {
            return new ResponseEntity<>(HttpStatus.NOT_FOUND);
